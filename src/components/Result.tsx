...
export default function Result({ constitution, onDiagnose, onShare }: Props) {
  ...
  return <section className="view result-view">
    <span className="eyebrow">생활 습관 기반 8체질 경향 자가 체크</span><div className="result-hero"><div><p>선택 경향이 가장 많이 나타난 유형은</p><h1>{constitutionLabels[constitution]}</h1><strong>{report.summary}</strong></div></div>
    ...
    <p className="disclaimer">이 결과는 선택지별 체질 경향을 단순 합산한 웰니스 자가 체크입니다. 사상체질은 한의사가 문진·맥진·체형·생활력 등을 종합해 판단하므로, 한의원 진단을 대체하거나 정확도를 보장하지 않습니다.</p>
    <RecommendedProducts constitution={constitution} />
  </section>
}
